...
\bm {s}(\bm { \theta} ^{(0)};(\bm {X}, Y=y)) =& \frac{1}{a( \phi)}(y - b'( \psi (\bm{X};\bm { \theta} ^{(0)})))\nabla _{ \bm { \theta}} \psi(\bm{X};\bm { \theta} ^{(0)})
\end{aligned}
\label{eqn:score_glm}
\end{align}
where the functions $b(\cdot)$ and $ \psi(\cdot)$ are some functions, the derivative $b'({\psi(\bm{X};\bm{\theta} ^{(0)})})$ is with respect to $ \psi(\cdot)$, and $ a ( \phi)$ is a positive scaling factor. In GLMs, $ g(E[Y=y|\bm{X}=\bm{x}]) = \psi ( \bm{X}=\bm{x};\bm { \theta})=\bm {x}^T\bm { \theta}$ is called the canonical link function. Using a Taylor expansion, the expected score function with the parameter change $ \Delta \bm { \theta}= \bm { \theta}^{(1)}-\bm { \theta}^{(0)}$ being small is:
\begin{align}
\begin{aligned}
...
\centering
 ...
 \begin{subfigure}[t]{0.4\linewidth}
	 ...
	 \includegraphics[width = \textwidth, trim=.2in .2in .7in .45in, clip]{../figures/v14/demons_fig/2D_logi.png}
	    \captionsetup{width=.9\linewidth}
         \caption{The shaded areas from Figure~\ref{fig:logi_err_rate_unch_a} and~\ref{fig:logi_err_rate_unch_b} are overlapped and it shows unchanged error rates.}
...
\end{align}
where $\bar {\bm{s}}$ and $\hat {\bm {\Sigma}}$ are the sample mean vector and covariance matrix of $\bm {s}_t$, respectively, estimated from some set of training data. If $T_t^2$ at some $t$ exceeds a specified upper control limit (UCL), this is taken to be an indication that $P(Y|\bm{X})$ in the time-vicinity of observation $t$ has changed.     
...
\centering
 \begin{subfigure}[t]{0.6\linewidth}
         \centering
         \includegraphics[width = 1\linewidth, trim=1.55in .85in 1.55in .85in, clip]{../figures/v14/flow_chart/Retrospective_1.png}
         \caption{Retrospective Analysis.}
         \label{fig:retro_analysis}
  \end{subfigure}
  \begin{subfigure}[t]{0.6\linewidth}
         \centering
         \includegraphics[width = 1\linewidth, trim=1.55in .85in 1.55in .85in, clip]{../figures/v14/flow_chart/Monitoring_1.png}
         \caption{Monitoring.}
         \label{fig:Monitoring}
  \end{subfigure}
  \begin{subfigure}[t]{0.6\linewidth}
         \centering
         \includegraphics[width = 1\linewidth, trim=1.55in .85in 1.55in .85in, clip]{../figures/v14/flow_chart/Diagnose_1.png}
         \caption{Diagnosis.}
         \label{fig:diagnosis}
  \end{subfigure}
...
Here, we revisit the illustrative example of simple logistic regression introduced in Section~\ref{ss:score_func} to complete the discussion on how we use MEWMA to resolve the limitation error-based methods have. According to the equation~(\ref{eqn:logi_mod_score}), the Hotelling $T^2$ for the logistic regression model can be written as:
\begin{align}
T_t^2 = (y-p^{(0)}(x))^2 \bm{X}^T\hat {\bm { \Sigma}} ^{(0)-1}\bm{X}
\label{eqn:logi_hotellingt2}
\end{align}
where the notations follow the equation~(\ref{eqn:logi_mod_score}) and~(\ref{eqn:simp_nota_p}). Similarly to the equation~(\ref{eqn:logi_err_rate}), we can take expectation to the quantity above to evaluate the change of monitoring statistics before and after concept drift, which we refer to as deviation function $dev(\cdot)$.
\begin{align}
\begin{aligned}
...
\end{align}
Notice that here we substitute $\bm{\Sigma}^{(0)}=E[p^{(0)}(1-p^{(0)})\bm{XX}^T]$ for the estimated covariance $\hat{\bm{\Sigma}}^{(0)}$ in the equation~(\ref{eqn:hotellingt2}) to simplify the analysis. After concept drift, assuming that the covariance matrix does not change much before and after concept drift ($\bm{\Sigma}^{(1)}\approx\bm{\Sigma}^{(0)}$), the deviation function can be decomposed as $dev(\bm{\theta}^{(1)})=E[Tr(p^{(1)}(1-p^{(1)})\bm{XX}^T\bm{\Sigma}^{(0)-1})]+E[(p^{(1)}-p^{(0)})^2\bm{X}^T\bm{\Sigma}^{(0)-1}\bm{X}]=E[Tr(\bm{\Sigma}^{(1)}\bm{\Sigma}^{(0)-1})]+E[(p^{(1)}-p^{(0)})^2\bm{X}^T\bm{\Sigma}^{(0)-1}\bm{X}]\approx dev(\bm{\theta}^{(0)})\\+E[(p^{(1)}-p^{(0)})^2\bm{X}^T\bm{\Sigma}^{(0)-1}\bm{X}]\gtrapprox dev(\bm{\theta}^{(0)})$, where $Tr(\cdot)$ is the trace of a matrix. This can be visualized in Figure~\ref{fig:logi_med_penal}. The shaded areas under curves represent the monitoring statistics. As shown in Figure~\ref{fig:logi_err_rate_penal} of the error rate of the error-based method, it does not change before and after concept drift; while in Figure~\ref{fig:logi_score_rate_penal} of the expected deviation of the score-based method, it increases after concept drift, which would be detected using our MEWMA monitoring method. After detection, the model can be updated to improve the performance as in Figure~\ref{fig:logi_err_rate_unch_d}. In Section~\ref{s:demon_cd}, a numerical example would be presented to further support the analysis here.
...
One of the advancement of machine learning is that models become increasingly complex. Some state-of-the-art models can have millions of parameters, like convolutional neural network. With such high-dimension of parameters, the sample covariance matrix in the equation~(\ref{eqn:hotellingt2}), $\hat {\bm { \Sigma}}$, is very likely to be close to singular. For example, when the sample size of our training data (denoted as $n_1$ as in Section~\ref{ss:MEWMA}) is smaller than the dimension of parameters, $dim(\bm { \theta})$, the sample covariance would be singular. 
...
\end{align}
{According to the definition, a concept drift corresponds to change in $E _{ \bm { \theta} }[\bm{s} (\bm { \theta} ^{ (0)};(\bm {X}, Y))| \bm {X}]$ from $\bm{0}$, because this indicates the change in $P(Y|\bm {X};\bm{\theta})$. In practice, we usually can not directly monitor $E _{ \bm { \theta} }[\bm{s} (\bm { \theta} ^{ (0)};(\bm {X}, Y))| \bm {X}]$, because of the lack of control in randomness from $\bm {X}$. Instead, we actually monitor $E_{ \bm { \theta} }[\bm{s} (\bm { \theta} ^{ (0)};(\bm {X}, Y))]$. This actually makes sense, because the {drift} of the left-hand-side in the equation~(\ref{eqn:joint_expe}) implies the {drift} of the inner expectation of the right-hand-side, meaning concept drift; the reverse is generally true except that {$E _{ \bm { \theta} }[\bm{s} (\bm { \theta} ^{ (0)};(\bm {X}, Y))| \bm {X}]$} has a specific form and non-zero values at different realizations of $\bm {X}$ cancel out after taking an expectation with respect to $\bm {X}$. As shown in Section~\ref{ss:score_func}, under assumptions of small changes of and smooth conditions with respect to the parameter, for generalized linear models, concept drift in parametric models implies non-zero mean of the score function.}
