...
\centering
\includegraphics[width = 1\linewidth, trim=.35in .69in .35in .69in, clip]{../figures/v14/flow_chart/Retrospective_1.png}
\caption{The framework of monitoring/detecting concept drift based on the score function. Conducting retrospective analysis using MEWMA and/or score function clustering to ensure there is no significant concept drift in the data set used to train and set control limits. The detailed procedure is following. The data set $\{\bm{x}_i, y_i\}_{i=1}^m$ was collected in time order. The entire data set is used to train a model and inspected retrospectively for significant concept drift. The two methods, MEWMA and score clustering can be used for this inspection. If no significant concept drift is spotted as shown in the above case, we can move to the next step. Otherwise, as shown in the below case, we can go back to the beginning and the size of data set can be recursively reduced by removing unstable data points if significant concept drift exists.}
