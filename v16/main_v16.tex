...
\end{align}
Notice that here we substitute $\bm{\Sigma}^{(0)}=E[p^{(0)}(1-p^{(0)})\bm{XX}^T]$ for the estimated covariance $\hat{\bm{\Sigma}}^{(0)}$ in the equation~(\ref{eqn:hotellingt2}) to simplify the illustration. After concept drift, assuming that the covariance matrix does not change much before and after concept drift ($\bm{\Sigma}^{(1)}\approx\bm{\Sigma}^{(0)}$), the deviation function can be decomposed as $dev(\bm{\theta}^{(1)})=E[Tr(p^{(1)}(1-p^{(1)})\bm{XX}^T\bm{\Sigma}^{(0)-1})]+E[(p^{(1)}-p^{(0)})^2\bm{X}^T\bm{\Sigma}^{(0)-1}\bm{X}]=E[Tr(\bm{\Sigma}^{(1)}\bm{\Sigma}^{(0)-1})]+E[(p^{(1)}-p^{(0)})^2\bm{X}^T\bm{\Sigma}^{(0)-1}\bm{X}]\approx dev(\bm{\theta}^{(0)})+E[(p^{(1)}-p^{(0)})^2\bm{X}^T\bm{\Sigma}^{(0)-1}\bm{X}]$, where $Tr(\cdot)$ is the trace of a matrix. This can be visualized in Figure~\ref{fig:logi_med_penal}. The shaded areas under curves represent the monitoring statistics. As shown in Figure~\ref{fig:logi_err_rate_penal} of the error rate of error-based method, it does not change before and after concept drift; while in Figure~\ref{fig:logi_score_rate_penal} of the expected deviation of the score-based method, it increases after concept drift, which would be detected using our MEWMA monitoring method. After detection, the model can be updated to improve the performance. In Section~\ref{s:demon_cd}, a numerical example would be presented to further support analysis here.
