...
\item
\textit{We show empirically that concept drift doesn't necessarily result in change in error as illustrated in Figure~\ref{fig:logi_err_rate_unch}, but sufficiently changes the mean of score-based metrics to be non-zero;}
\item
...
Besides those advantages from methodological perspective, the score-based method also incorporates practical and computational advantages from the state-of-the-art statistical process control (SPC) methods and stochastic gradient descents (SGD).

\begin{itemize}
\item
\textit{The state-of-the-art statistical process control (SPC) methods are used in monitoring the mean of the score functions, so that this study opens up the possibility of any type of mean-change monitoring methods.}
\item
...
Because of those nice properties in methodological and practical perspective, the score-based method is a very general and can be applied onto any models which can obtain derivatives with respect to parameters, like generalized linear models or neural network; it can borrow power of any mean monitoring methods from decades of research in the field of SPC; and integration of this method into existing systems would not be computationally expensive. 

Practically, the score-based method would be used in following ways. First, it can justify the stationarity assumption in training models, retrospectively. Then, it can provide quantitative guidance on when the model should be updated or retrained, on occurrence of a drift. Third, insights obtained from diagnosis with this method offer interpretation to the nature of drifts, which can be highly valuable in many industries (e.g. business, insurance, and health care, etc). 
