...
\label{eqn:logi_err_rate}
\end{align}
Note that this classifier $\hat{y}(x)$ minimizes the unconditional error rate. When $\bm{\theta} = \bm{\theta}^{(0)}$ so the true predictive relationship $P(Y=1|\bm{X})$ is as in Figure~\ref{fig:logi_err_rate_unch_a}, the error rate is $27.6\%$. Now suppose we use the same classifier based on $\bm{\theta}^{(0)}$, but $\bm{\theta}$ changes to $\bm{\theta}^{(1)}$ so the true predictive relationship $P(Y=1|\bm{X})$ is as in Figure~\ref{fig:logi_err_rate_unch_b}. Even though $\bm{\theta}$ has changed, the error rate remains unchanged at $27.6\%$. Clearly, any concept drift detection method based on the error rate will fail to detect the change in $\bm{\theta}$. It is important to note that although the error rate does not change when $\bm{\theta}$ changes from $\bm{\theta}^{(0)}$ to $\bm{\theta}^{(1)}$, the change in $\bm{\theta}$ constitutes an opportunity to improve the classification accuracy if we were to detect the change and update the classification rule based on the new model, i.e., use the new classifier $\hat{y}(x)=1$ if $x \geq 0.5575$, which corresponds to $\hat{y}(x)=1$ if $p^{(1)}(x)\geq 0.5$ (see Figure~\ref{fig:logi_err_rate_unch_d}).
...
\label{eqn:dec_func}
\end{align}
After concept drift, the data generating process has a conditional distribution as the red solid line in Figure~\ref{fig:logi_err_rate_unch_b}. Without updating the model, the decision boundary remains the same, even though the optimal decision boundary has been changed. The (unconditional) error rate before and after the concept drift are shown as the blue and yellow shaded areas (with the distribution of $X$ as the weight across $x$ axis) in Figures~\ref{fig:logi_err_rate_unch_a} and~\ref{fig:logi_err_rate_unch_b}, and corresponds to the cases of $j=0$ and $j=1$ in Equation~(\ref{eqn:logi_err_rate}), respectively. 

To intuitively compare the (weighted) size of two shaded areas and simplify figures, we can imagine overlapping Figures~\ref{fig:logi_err_rate_unch_a} and~\ref{fig:logi_err_rate_unch_b}, where we can see after concept drift the optimal decision boundary $x=0$ of the old model, which perfectly captures the original data generating process, has a smaller error rate in the region $x<0$ but a larger error rate in the region $x>=0$, resulting in net zero change in the overall unconditional error rate, which will be later shown again together in Figure~\ref{fig:logi_err_rate_penal}. In other words, error-based methods would fail to detect the concept drift in this case. Instead, if this concept drift were detected, the decision boundary could be updated as in Figure~\ref{fig:logi_err_rate_unch_d} bringing us a smaller unconditional error rate. 
...
\end{align}
where the notations follow Equation~(\ref{eqn:logi_mod_score}) and~(\ref{eqn:simp_nota_p}). Similarly to Equation~(\ref{eqn:logi_err_rate}), we can take expectation to the quantity above to evaluate the change of monitoring statistics before and after concept drift, which we refer to as the expected Hotelling $T^2$ $dev(\cdot)$.
% \begin{align}
% \begin{aligned}
