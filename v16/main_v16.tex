...
The purpose of Step $3$, depicted in Figure~\ref{fig:diagnosis}, is to conduct a diagnostic analysis to help determine the nature of the concept drift, if any drift is detected in Phase-II. This involves plotting the $T_t^2$ statistic v.s. $t$ over the data set of interest, which is depicted in the top plots in Figure~\ref{fig:diagnosis}. Note that the $T^2$ statistic of MEWMA chart aggregates changes in any of the components of the score vector into a single scalar statistic. To provide richer diagnostic information and help understand which parameters have changed, analogous univariate EWMA control charts for (transformed) individual components of the score vector should also be constructed, which is depicted in the bottom plots in Figure~\ref{fig:diagnosis}. We describe these univariate control charts in Section~\ref{s:decou_cd}.
...
% \label{ss:score_regu}
Another related issue is regularization of complex models, which is almost always required to combat overfitting. The regularization term is used to penalize complex models and large parameters, by minimizing, for example, the negative log-likelihood plus the $L_2$ norm of all parameters: $l(\bm{\theta})=-\frac{1}{n}\sum_{i=1}^n \log P(y_i|\bm{x}_i;\bm{\theta})+\frac{c}{2n}||\bm{\theta}||_2^2$, where $c>0$ is a regularization parameter. In this case we can redefine the score function by noting that $\nabla_{\bm{\theta}}l(\bm{\theta})=-\frac{1}{n}\sum_{i=1}^n(\bm{s}(\bm{\theta};(\bm{x}_i,y_i))-\frac{c}{n}\bm{\theta})=-\frac{1}{n}\nabla_{\bm{\theta}}\sum_{i=1}^n\log(P(y_i|\bm{x}_i;\bm{\theta})\exp\{-\frac{c}{2n}||\bm{\theta}||_{2}^2\})$. Thus, if we redefine the score function as $\bm{s}(\bm{\theta}; (\bm{x}_i, y_i))-\frac{c}{n}\bm{\theta}$, this can be viewed as replacing the likelihood with its Bayesian counterpart $P(Y|\bm{X};\bm{\theta})\exp\{-\frac{c}{2n}||\bm{\theta}||_{2}^2\}$, which incorporates a prior distribution (in this case, a Gaussian prior for $L_2$ regularization) on the parameters. The MEWMA and Hotelling $T^2$ computations in the concept drift monitoring procedure would then be exactly the same but with the score vector replaced by $\bm{s}(\bm{\theta}; (\bm{x}_i, y_i))-\frac{c}{n}\bm{\theta}$. In some popular form of regularization, this would not change the monitoring statistics. For example, the above $L_2$ regularization only subtracts a constant vector to all score vectors: $\frac{c}{n}\bm{\theta}$, which would cancel after substracting the mean of the score vectors in Equation~(\ref{eqn:hotellingt2}).
...
\begin{subfigure}[t]{0.374\linewidth}
     ...
     \captionsetup{width=.95\linewidth}
     \caption{Prospective Diagnosis plots for a model trained using normalized bike rental counts (using yearly mean). The training data is from $2013$-Jan to $2016$-Dec; Phase-I data is from the whole year of $2017$; Phase-II data is after $2018$-Jan. In estimating the control limits in this figure, the $\bar{\bm{s}}$ in Equation~(\ref{eqn:hotellingt2}) is estimated from data of the Phase-I, while the $\hat{\bm{\Sigma}}$ is estimated from training data.}
     \label{fig:bs_norm_cnt_2}
\end{subfigure}
\caption{
}
\label{fig:bike_sharing_diag}
