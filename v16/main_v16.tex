...
\end{align}
where $\bar {\bm{s}}$ and $\hat {\bm {\Sigma}}$ are the sample mean vector and covariance matrix of $\bm {s}_t$, respectively, estimated from some set of training data. If $T_t^2$ at some $t$ exceeds a specified (chosen as described in Section \ref{ss:imp_score_method}) upper control limit (UCL), this is taken to be a indication that $P(Y|\bm{X})$ in the time-vicinity of observation $t$ has changed.     
